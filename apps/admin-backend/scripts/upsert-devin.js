...
async function applyChanges(db) {
  ...
  for (const plan of plans) {
    const common = {
      ...
      modalDescription: `${plan.title}\nПодключение на ваш аккаунт Devin со входом по логину и паролю.\nНе передавайте коды 2FA и резервные коды заранее.\nПосле подключения рекомендуем сменить пароль.`,
      modalDescriptionEn: `${plan.titleEn}\nActivation on your Devin account with your login and password.`,
      price: plan.price,
      currency: "RUB",
  ...
  const servicePage = await db.servicePage.upsert({
    ...
    create: {
      ...
      constructorDescription: "Выберите Pro, Max или Teams. После оплаты мы подключим подписку на ваш аккаунт Devin и сообщим о результате.",
