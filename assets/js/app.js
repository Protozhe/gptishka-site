...
(() => {
  ...
  function getVisualConfig(item) {
    ...
    return {
      ...
      textColor: String(visual.textColor || "").trim(),
      buttonText: String(visual.buttonText || TEXT.payNow).trim(),
      buttonStyle: String(visual.buttonStyle || "primary").trim(),
      buttonBackground: String(visual.buttonBackground || "").trim(),
      buttonTextColor: String(visual.buttonTextColor || "").trim(),
  ...
  function getShowcaseButtonBackground(visual) {
    const value = String(visual.buttonBackground || "").trim();
    if (value) return value;
    const style = String(visual.buttonStyle || "primary").trim().toLowerCase();
    if (style === "secondary" || style === "glass") return "rgba(255,255,255,.14)";
    if (style === "dark") return "linear-gradient(135deg,#111827,#020617)";
    return "linear-gradient(135deg,#35f28f,#18c878,#0f8f5c)";
  }

  function getShowcaseButtonTextColor(visual) {
    const value = String(visual.buttonTextColor || "").trim();
    if (value) return value;
    const style = String(visual.buttonStyle || "primary").trim().toLowerCase();
    return style === "primary" || !style ? "#06110b" : "#ffffff";
  }
  ...
  function buildShowcaseProductCard(item, index) {
    ...
    const buttonStyle = String(visual.buttonStyle || "primary").trim().toLowerCase().replace(/[^a-z0-9_-]+/g, "-") || "primary";
    const cardStyle = [
      "--showcase-card-bg:" + background,
      visual.textColor ? "--showcase-card-text:" + visual.textColor : "",
      "--showcase-button-bg:" + getShowcaseButtonBackground(visual),
      "--showcase-button-color:" + getShowcaseButtonTextColor(visual),
    ].filter(Boolean).join(";");
    ...
    return (
      '<article class="product-showcase-card product-showcase-card--button-' + escapeHtml(buttonStyle) + '" style="' + escapeHtml(cardStyle) + '"' +
  ...
  async function fetchProductsPayload(forceRefresh = false) {
    ...
    function normalizePayload(payload) {
      ...
      if (Array.isArray(source.sections)) {
        const byId = new Map();
        const serviceCards = Array.isArray(source.serviceCards) ? source.serviceCards : [];
        const sections = source.sections
          .map(section => {
            ...
            return {
              ...section,
              products,
              serviceCards: Array.isArray(section?.serviceCards) ? section.serviceCards : serviceCards,
            };
          })
          .filter(section => Array.isArray(section.products) && section.products.length);
        return { ...source, sections, items: Array.from(byId.values()), serviceCards };
      }
  ...
  function applyServicePageContent(payload) {
    ...
    if (key === "chatgpt" && !isEnPage) {
      document.title = "ChatGPT Plus в России — покупайте у нас! | GPTишка";
  ...
  function shouldForceProductCards(section) {
    return String(section?.renderMode || "").trim().toLowerCase() === "cards";
  }

  function getShowcaseServiceCardConfig(section, serviceKey) {
    const key = normalizeAiServiceKey(serviceKey);
    if (!key) return null;
  ...
  function getServiceCardBackground(serviceCard, fallbackVisual) {
    if (!serviceCard) return getShowcaseCardBackground(fallbackVisual);
    const backgroundType = String(serviceCard.backgroundType || "").trim().toLowerCase();
    if (backgroundType === "gradient" && serviceCard.backgroundGradient) return serviceCard.backgroundGradient;
    if (backgroundType === "image" && serviceCard.imageUrl) {
      return "linear-gradient(180deg, rgba(0,0,0,.10), rgba(0,0,0,.62)), url('" + String(serviceCard.imageUrl).replace(/'/g, "%27") + "') center/cover";
    }
    if (serviceCard.backgroundColor) return serviceCard.backgroundColor;
    return getShowcaseCardBackground(fallbackVisual);
  }
  ...
  function shouldRenderTopupsDirectory(section) {
    const products = Array.isArray(section?.products) ? section.products : [];
    if (!products.length) return false;
    const sectionText = [
      section?.slug,
    ...
    if (sectionText.includes("пополн") || sectionText.includes("topup") || sectionText.includes("top-up")) {
      return products.some(isTopupsProduct);
    }
    const topupsCount = products.filter(isTopupsProduct).length;
    return topupsCount >= 1 && topupsCount / products.length >= 0.6;
  }

  function renderTopupsDirectoryCard(section) {
    const items = (Array.isArray(section?.products) ? section.products : []).filter(isTopupsProduct);
    ...
    return (
      '<div class="ai-directory-grid ai-directory-grid--topups">' +
        '<article class="ai-directory-card ai-directory-card--integrated ai-directory-card--steam ai-directory-card--' + escapeHtml(displayTheme) + '" style="--ai-directory-bg:' + escapeHtml(displayBackground) + '">' +
          ...
          '<div class="ai-directory-card__body">' +
            '<div class="ai-directory-card__top">' +
              '<span class="ai-directory-card__icon ai-service-card__icon--' + escapeHtml(displayTheme) + '">' + escapeHtml(displayIconText) + "</span>" +
              '<span class="ai-directory-card__count">' + escapeHtml(String(items.length)) + "</span>" +
            "</div>" +
            '<h4 class="ai-directory-card__name">' + escapeHtml(displayTitle) + "</h4>" +
            '<p class="ai-directory-card__desc">' + escapeHtml(displayDescription) + "</p>" +
  ...
  function renderVpnDirectoryCard(section) {
    ...
    const serviceCard = getShowcaseServiceCardConfig(section, "vpn");
  ...
  function renderAiDirectoryCard(group, section) {
    const serviceKey = normalizeAiServiceKey(group?.service?.key);
    const serviceCard = getShowcaseServiceCardConfig(section, serviceKey);
  ...
  function renderAiServiceDirectory(section, groups, sectionIdx) {
    ...
    const otherCards = (Array.isArray(section?.products) ? section.products : [])
      ...
      .join("");
    return (
      '<div class="ai-directory-grid">' + groups.map(group => renderAiDirectoryCard(group, section)).join("") + "</div>" +
  ...
  function renderShowcaseSections(sections) {
    ...
    return safeSections.map((section, sectionIdx) => {
      ...
      const forceProductCards = shouldForceProductCards(section);
      const renderAsAiDirectory = !forceProductCards && shouldRenderAiServiceDirectory(section, aiGroups);
      const renderAsVpnDirectory = !forceProductCards && !renderAsAiDirectory && shouldRenderVpnDirectory(section);
      const renderAsTopupsDirectory = !forceProductCards && !renderAsAiDirectory && !renderAsVpnDirectory && shouldRenderTopupsDirectory(section);
      const sectionTitle = renderAsAiDirectory
        ? (isEnPage ? "AI Services" : "Нейросети")
        : renderAsVpnDirectory
          ? "VPN"
        : renderAsTopupsDirectory
          ? (isEnPage ? "Top-ups" : "Пополнения")
      ...
      const sectionDescription = renderAsAiDirectory
        ...
        : renderAsTopupsDirectory
          ? (isEnPage
              ? "Choose a top-up product and proceed to payment."
              : "Выберите товар для пополнения и перейдите к оплате.")
      ...
      const cardsMarkup = renderAsAiDirectory
        ...
        : renderAsTopupsDirectory
          ? renderTopupsDirectoryCard(section)
        : section.products
            .map((item, itemIdx) => buildShowcaseProductCard(item, sectionIdx * 100 + itemIdx))
            .filter(Boolean)
            .join("");
      if (!cardsMarkup) return "";
      const isCollapsible = !renderAsAiDirectory && !renderAsVpnDirectory && !renderAsTopupsDirectory && section.products.length > 10;
  ...
  function renderServiceConstructorCard(item, serviceKey) {
    ...
    return (
      ...
      ' data-badge="">' +
        ...
        ' data-plan-key="' + escapeHtml(planKey) + '"' +
        ' data-delivery-key="' + escapeHtml(deliveryKey) + '"' +
        ' data-duration-key="' + escapeHtml(durationKey) + '">' + escapeHtml(isEnPage ? "Buy" : "Купить") + "</button>" +
      "</div>"
    );
  }
  ...
  function updateChatGptGoOrderTotals(form, item) {
    ...
    if (discountRow) {
      ...
      if (valueNode) valueNode.textContent = "в€’" + formatPriceByCurrency(discount, currency);
    }
    ...
    try {
      ...
      if (isValid) {
        ...
        setChatGptGoPromoMessage(
          form,
          TEXT.promoAccepted + (discount > 0 ? " (в€’" + formatPriceByCurrency(discount, String(item.currency || "RUB")) + ")" : ""),
          "success"
        );
        trackAnalyticsEvent("promo_validate_success", {
    ...
    return (
      ...
      ' data-badge="">' +
        '<div class="chatgpt-order-scroll">' +
          ...
          '<section class="chatgpt-order-section"><div class="chatgpt-order-section__head"><h4 class="chatgpt-order-section-title">Оплата</h4><p>Выберите платёжный шлюз</p></div><div class="chatgpt-order-payment chatgpt-payment-options" role="radiogroup" aria-label="Способ оплаты">' + renderChatGptPaymentOption("lava", "LAVA", "/assets/img/payment-lava.svg?v=20260724-lava-mark2", savedPaymentMethod) + renderChatGptPaymentOption("enot", "ENOT", "/assets/img/payment-enot.svg?v=20260724-enot-mark1", savedPaymentMethod) + '</div><p class="chatgpt-order-error" data-chatgpt-go-error-for="paymentMethod"></p><details class="chatgpt-order-collapsible chatgpt-order-processing-details"><summary>Сроки выполнения заказа</summary><p>Мы обрабатываем заказы ежедневно с 08:00 до 20:00 по МСК. Заказы с автоматическим подключением выполняются 24/7. Среднее время ожидания — от 5 минут до 2 часов после оплаты.</p></details></section>' +
  ...
  function isSteamTopupItem(item) {
    if (!item) return false;
    const values = [
      item.product,
      item.productId,
    ...
    return values.some(value =>
      ...
      value.includes("mann") ||
      value.includes("manco") ||
      value.includes("mannco") ||
      value.includes("mann-co") ||
      value.includes("steam-topup") ||
      value.includes("steam_topup") ||
      value.includes("topup-steam") ||
      value.includes("\u043c\u0430\u043d\u043d") ||
      value.includes("\u043f\u043e\u043f\u043e\u043b\u043d\u0435\u043d\u0438\u0435 steam")
    );
  }

  function getSteamTopupPath() {
    return isEnPage ? "/en/store/steam/topup/" : "/store/steam/topup/";
  }

  function openDirectProductRouteIfNeeded(item) {
    if (isSteamTopupItem(item)) {
      navigateWithPageTransition(getSteamTopupPath());
      return true;
    }
    return false;
  }
  ...
  document.addEventListener("click", e => {
    ...
    if (payNowBtn) {
      ...
      const item = getCardItem(card);
      if (!item) return;
      if (openDirectProductRouteIfNeeded(item)) return;
...
(() => {
  ...
  function isPublicPage() {
    ...
    return !/^\/admin(?:\/|$)/.test(path) && !/^\/api(?:\/|$)/.test(path);
  }

  function ensureStyles() {
    if (document.getElementById("gptishka-compliance-style")) return;
    const style = document.createElement("style");
    style.id = "gptishka-compliance-style";
    style.textContent = `
      ...
      .gptishka-compliance-note strong {
        display: block;
        margin-bottom: 4px;
  ...
  function updatePublicMeta() {
    ...
    const isChatgpt = /\/chatgpt(?:\.html)?\/?$/.test(path);

    if (isHome) {
      const title = en
        ? "GPTishka — independent digital services marketplace"
        : "GPTishka — независимый магазин цифровых услуг";
      const description = en
        ? "Independent digital services marketplace: AI subscription assistance, activation support, Steam top-ups and V*N access. GPTishka is not affiliated with OpenAI, Anthropic or xAI."
        : "Независимый магазин цифровых услуг: помощь с AI-подписками, активацией, Steam-пополнением и V*N-доступом. GPTishka не связан с OpenAI, Anthropic или xAI.";
    ...
    if (isChatgpt) {
      const title = en
        ...
        : "ChatGPT Plus в России — покупайте у нас! | GPTишка";
      const description = en
        ...
        : "ChatGPT Plus в России: покупайте у GPTишка с оплатой в рублях, подключением без передачи логина и пароля, поддержкой и гарантией.";
      document.title = title;
      setMeta("description", description);
      setMeta("og:title", title, "property");
      setMeta("og:description", description, "property");
  ...
  function createComplianceNote() {
    const en = isEnglishPage();
    const note = document.createElement("section");
    note.className = "gptishka-compliance-note";
    note.setAttribute("data-compliance-version", VERSION);
    note.innerHTML = en
      ? `<strong>Independent service notice</strong>GPTishka is an independent digital services marketplace. We are not affiliated with ${PRODUCT_OWNERS} or other trademark owners. Product names are used only to describe user-requested compatible services. If anything looks unclear, contact <a href="${SUPPORT_URL}" target="_blank" rel="noopener">support</a>.`
      : `<strong>Уведомление о независимом сервисе</strong>GPTishka — независимый магазин цифровых услуг. Мы не являемся официальным представителем ${PRODUCT_OWNERS} или других правообладателей. Названия продуктов используются только для описания совместимых услуг, запрошенных пользователями. Если что-то выглядит непонятно — напишите в <a href="${SUPPORT_URL}" target="_blank" rel="noopener">поддержку</a>.`;
    return note;
  }

  function insertComplianceNote() {
    if (document.querySelector(".gptishka-compliance-note")) return;
    const footer = document.querySelector("footer");
    const main = document.querySelector("main");
    const note = createComplianceNote();
  ...
  function repairHeaderText() {
    ...
    document.querySelectorAll(".lang-current span, .lang-item span").forEach((node) => {
      ...
      } else if (text.includes("рус") || text.includes("сѓ") || text.includes("ru")) {
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initComplianceLayer, { once: true });
  } else {
    initComplianceLayer();
  }
})();
