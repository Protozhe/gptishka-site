...
(() => {
  ...
  function getAnchorVisualShift(targetId) {
    ...
    if (key !== "pricing" && key !== "how" && key !== "faq" && key !== "payment") return 0;
    const isCompact = window.matchMedia && window.matchMedia("(max-width: 760px)").matches;
    if (isCompact) return 0;
    const shift = Math.round(window.innerHeight * 0.15);
    return Math.max(24, Math.min(84, shift));
  }
  ...
  function alignToHashTarget(behavior = "auto") {
    ...
    if (isCompact && targetId.toLowerCase() === "payment") {
      const howSection = target.closest("section.how") || document.querySelector("section.how");
      if (howSection) {
        anchorTarget = howSection;
      }
    }
