...
(() => {
  ...
  function getStickyAnchorOffset(targetId) {
    const header = document.querySelector("header");
    const headerHeight = header ? Math.ceil(header.getBoundingClientRect().height) : 0;
    const isCompact = window.matchMedia && window.matchMedia("(max-width: 760px)").matches;
    const baseGap = isCompact ? 10 : 12;
    const extraGap = targetId === "pricing"
      ? 0
      : 0;
    return headerHeight + baseGap + extraGap;
  }

  function getAnchorVisualShift(targetId) {
    const key = String(targetId || "").toLowerCase();
    const isCompact = window.matchMedia && window.matchMedia("(max-width: 760px)").matches;

    if (isCompact) {
      if (key === "payment") {
        const paymentShift = Math.round(window.innerHeight * 0.2);
        return Math.max(24, Math.min(72, paymentShift));
      }

      if (key === "pricing" || key === "how" || key === "faq") {
        const mobileShift = Math.round(window.innerHeight * 0.11);
        return Math.max(14, Math.min(44, mobileShift));
      }
    ...
    if (key === "payment") {
      const desktopPaymentShift = Math.round(window.innerHeight * 0.08);
      return Math.max(14, Math.min(38, desktopPaymentShift));
    }

    if (key === "pricing" || key === "how" || key === "faq") {
      const desktopShift = Math.round(window.innerHeight * 0.06);
      return Math.max(10, Math.min(28, desktopShift));
    }
  ...
  function alignToHashTarget(behavior = "auto") {
    ...
    const offsetTop = getStickyAnchorOffset(targetId);
    const visualShift = getAnchorVisualShift(targetId);
    const nextTop = Math.max(0, Math.round(targetTop - offsetTop + visualShift));
  ...
  function isIndexAliasPath(pathname) {
    const normalized = normalizePathname(pathname);
    return normalized === "/" || normalized === "/index.html";
  }

  function isEnIndexAliasPath(pathname) {
    const normalized = normalizePathname(pathname);
    return normalized === "/en" || normalized === "/en/index.html";
  }

  function resolveSamePageAnchorUrl(href) {
    const rawHref = String(href || "").trim();
    ...
    const targetSearch = String(targetUrl.search || "");
    if (targetSearch && targetSearch !== currentSearch) return null;
    const samePath = currentPath === targetPath;
    const sameRuIndex = isIndexAliasPath(currentPath) && isIndexAliasPath(targetPath);
    const sameEnIndex = isEnIndexAliasPath(currentPath) && isEnIndexAliasPath(targetPath);

    return samePath || sameRuIndex || sameEnIndex ? targetUrl : null;
  }
  ...
  document.addEventListener("click", e => {
    ...
    const samePageUrl = resolveSamePageAnchorUrl(link.getAttribute("href"));
    if (!samePageUrl) return;

    e.preventDefault();
    const targetHash = String(samePageUrl.hash || "").trim();
  ...
  if (window.location.hash) {
    window.setTimeout(() => alignToHashTarget("auto"), 0);
    window.setTimeout(() => alignToHashTarget("auto"), 180);
    window.setTimeout(() => alignToHashTarget("auto"), 420);
  }
...
(() => {
  ...
  function ensureTickerTotalLabel() {
    if (!totalLabelEl) return;
    const fallback = String(TEXT.totalLabel || "").trim() || (isEnPage ? "total activations" : "всего активаций");
    const normalized = String(totalLabelEl.textContent || "")
      .replace(/:+\s*$/g, "")
      .trim();
    totalLabelEl.textContent = `${normalized || fallback}:`;
  }
  ...
  function createTicker() {
    ...
    totalLabelEl = ticker.querySelector(".site-ticker__stat-label");
    ensureTickerTotalLabel();
  }
