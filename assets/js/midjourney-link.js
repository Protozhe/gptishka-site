(() => {
  ...
  const orderToken = String(params.get("t") || "").trim();
  ...
  function validLink(value) {
    ...
    try {
      ...
      return url.protocol === "https:" && url.hostname === "checkout.stripe.com" &&
        !url.username && !url.password && !url.port && /^\/c\/pay\/cs_live_[A-Za-z0-9]+$/.test(url.pathname);
    } catch {
      return false;
    }
  }
  ...
  async function loadOrder() {
    ...
    try {
      ...
      if (data.deliveryMode !== "payment_link" || !/^midjourney-(basic|standard|pro)-1$/.test(String(data.productSlug || ""))) {
        setStatus("Эта форма доступна только для заказа Midjourney.", "error");
        return;
      }
      plan.textContent = `Тариф: ${data.productTitle || "Midjourney"}`;
      form.hidden = false;
      if (data.status === "submitted") {
        setStatus("Ссылка уже получена. Мы оплатим подписку Midjourney в ближайшее время, ожидайте. Если возникнут сложности, менеджер свяжется с вами.", "success");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!validLink(link)) {
      setStatus("Нужна полная HTTPS-ссылка Stripe Checkout вида checkout.stripe.com/c/pay/cs_live_…", "error");
      input.focus();
      return;
    }
    ...
    try {
      ...
      input.value = "";
      button.textContent = "Отправить новую ссылку";
      setStatus("Ссылка получена. Мы оплатим подписку Midjourney в ближайшее время, ожидайте. Если возникнут сложности, менеджер свяжется с вами.", "success");
    } catch {
