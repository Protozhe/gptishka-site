(() => {
  ...
  async function loadOrder() {
    ...
    try {
      ...
      if (data.deliveryMode !== "payment_link" || !/^suno-(pro|premier)-1-month$/.test(String(data.productSlug || ""))) {
        setStatus("Эта форма доступна только для заказа Suno.", "error");
        return;
      }
      plan.textContent = `Тариф: ${data.productTitle || "Suno"}`;
      form.hidden = false;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      input.value = "";
      button.textContent = "Отправить новую ссылку";
      setStatus("Ссылка получена. Мы оплатим подписку Suno в ближайшее время, ожидайте. Если возникнут сложности, менеджер свяжется с вами.", "success");
    } catch {
