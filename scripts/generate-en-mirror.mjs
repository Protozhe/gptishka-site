...
const pages = [
  ...
  // Suno's English page is maintained directly: its payment-link instructions differ from the old mirror copy.
...
const translations = [
  ...
  ["Открыть тарифы Suno", "Open the Suno plans"],
  ["Suno Pro и Premier для создания музыки и вокала.", "Suno Pro and Premier for AI music creation."],
  ["Pro / Premier · 1 месяц", "Pro / Premier · 1 month"],
  ...
  ["Перед подключением PRO или MAX аккаунт должен находиться на бесплатном тарифе FREE.", "Before connecting PRO or MAX, the account must be on the FREE plan."],
  ["До оформления заказа проверьте аккаунт: отправьте любое сообщение в Claude на бесплатном тарифе. Если Claude принимает запрос и отвечает, явных ограничений на аккаунте нет.", "Before placing an order, test the account by sending any message in Claude on the free plan. If Claude accepts the request and responds, there are no apparent restrictions on the account."],
  ["Рекомендуемый возраст аккаунта — не менее 7 дней. Новые аккаунты чаще попадают под автоматические проверки безопасности Anthropic.", "The recommended account age is at least 7 days. New accounts are more likely to be subject to Anthropic's automated security checks."],
  ["Если аккаунт зарегистрирован недавно, рекомендуем подождать около 6 часов и некоторое время пользоваться бесплатной версией перед подключением подписки.", "If the account was registered recently, we recommend waiting about 6 hours and using the free version for a while before connecting a subscription."],
  ["Во время использования соблюдайте правила Claude. Частая смена IP-адресов и регионов, стороннее ПО, массовая регистрация или передача аккаунта другим людям повышают риск ограничений.", "Follow Claude's rules while using the service. Frequent IP address or region changes, third-party software, mass registration, or sharing the account with other people increase the risk of restrictions."],
  ["Для подключения могут потребоваться технические данные аккаунта, например Organization ID или API-ключ. Сайт покажет точную инструкцию для выбранного способа активации. Данные используются только для выполнения заказа.", "Technical account details such as an Organization ID or API key may be required. The site will show exact instructions for the selected activation method. The data is used only to complete the order."],
  ["Вы выбираете тариф PRO или MAX и оформляете заказ.", "Select a PRO or MAX plan and place an order."],
  ["После оплаты сайт покажет данные, необходимые именно для выбранного способа подключения.", "After payment, the site will show the details required for the selected connection method."],
  ["Мы активируем подписку. Обычно подключение занимает от 5 до 20 минут, но при дополнительной проверке сервиса срок может увеличиться.", "We activate the subscription. Connection usually takes 5–20 minutes, but additional service checks may increase the processing time."],
  ["После подключения вы проверяете доступ к функциям выбранного тарифа. При необходимости поддержка поможет проверить результат.", "After connection, check access to the selected plan's features. Support will help verify the result if necessary."],
  ["Мы отвечаем за корректное подключение подписки и устраняем проблемы, возникшие по нашей вине, в рамках", "We are responsible for connecting the subscription correctly and resolve problems caused by us under the"],
  ["Гарантия распространяется на случаи, когда оплаченная подписка перестала работать раньше заявленного срока по причине, связанной с подключением услуги.", "The warranty applies when a paid subscription stops working before the stated end date due to an issue related to the connection."],
  ["Запросы на подтверждение личности, KYC/ID Verification и другие антифрод-проверки выполняются непосредственно Anthropic и не зависят от GPTishka.", "Identity verification requests, KYC/ID Verification, and other anti-fraud checks are performed directly by Anthropic and are outside GPTishka's control."],
  ["Блокировка или ограничение самого аккаунта со стороны Anthropic не является гарантийным случаем, если оно связано с состоянием аккаунта, нарушением правил сервиса, частой сменой IP-адресов или регионов, сторонним ПО либо действиями владельца аккаунта.", "An account block or restriction imposed by Anthropic is not a warranty case when it is related to the account's condition, violation of service rules, frequent IP address or region changes, third-party software, or actions by the account owner."],
  ["Если до покупки аккаунт уже не принимал сообщения на бесплатном тарифе, такая проблема не относится к подключённой подписке.", "If the account could not send messages on the free plan before purchase, that issue is not related to the connected subscription."],
